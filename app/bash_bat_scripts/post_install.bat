...
IF EXIST "%PROJECT_ROOT%\setup.py" (
    ...
    IF ERRORLEVEL 1 (
        SET "FAILURE_MESSAGE=NucleiSky package installation failed."
        GOTO :fail
    )
...
"%PYTHON_EXE%" -c "from menuinst.api import install; import os; print(install(os.path.join(r'%PREFIX%', 'NucleiSky', 'notebook_launcher.json')))" >> "%LOG_FILE%" 2>&1
IF ERRORLEVEL 1 (
...
SET "DISPLAY_VERSION=0.1.1"
SET "PUBLISHER=CellMigrationLab"
echo Registering NucleiSky in Windows Apps list >> "%LOG_FILE%"
reg add "%ARP_KEY%" /v DisplayName /d "NucleiSky" /f >> "%LOG_FILE%" 2>&1
